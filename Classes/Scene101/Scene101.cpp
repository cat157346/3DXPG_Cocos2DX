...
bool Scene101::init()
{
	...
	//auto bean = Sprite::create("scene101/bean01.png"); 也可以
	bean->setScale(g_fScaleFactor); // 背景圖片設定成跟背景一樣大時，直接根據比例縮放即可
	bean->setRotation(30.0);
	bean->setPosition(330,593); // 位置通常放置在螢幕正中間
	bean->setColor(Color3B::WHITE);
	...
	auto label1 = Label::createWithBMFont("fonts/tt.fnt", "Scene 101");
	...
	auto label3 = Label::createWithBMFont("fonts/bx.fnt", str2);
	auto label4 = Label::createWithBMFont("fonts/bx.fnt", str3);
	...
	label4->setColor(Color3B(245, 264, 123));
