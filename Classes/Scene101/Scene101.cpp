...
bool Scene101::init()
{
	...
	this->bean = Sprite::create("scene101/bean01.png");  // 使用 create 函式,給予檔名即可
	bean->setPosition(Vec2(origin.x + visibleSize.width / 6, origin.y + visibleSize.height - 140 - size.height)); // 位置通常放置在螢幕正中間
	size = bean->getContentSize();
	Point pos = bean->getPosition();
